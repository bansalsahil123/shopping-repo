	...
	if request.user.is_authenticated:
		...
		if cart_product:
			for p in cart_product:
				...
				print("Tepamount",tempamount)

				amount+=tempamount
				print("amount",amount)
				totalamount = amount+shipping_amount
				print("totalamount",totalamount)
			return render(request, 'app/addtocart.html', {'carts':cart_product, 'amount':amount, 'totalamount':totalamount, 'totalitem':totalitem})
		else:
...
def mobile_filter(request, data=None):
	...
	for item in mobiles:
		...
		tmp['selling_price']=item.selling_price
		tmp['discounted_price'] = item.discounted_price
		tmp['brand']=item.brand
		tmp['category']=item.category
		tmp['product_image']=item.product_image.url
		mobiles_dict.append(tmp)
	print("Mobiles dicts",mobiles_dict)
	return JsonResponse({'mobiles':mobiles_dict})
